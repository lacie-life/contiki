...
;------------------------------------------------------------
;	../../core/net/rime/runicast.c:73: sent_by_stunicast(struct stunicast_conn *stunicast, int status, int num_tx)
;	-----------------------------------------
...
_sent_by_stunicast:
	...
	mov	r6,dph
	mov	r7,b
;	../../core/net/rime/runicast.c:75: struct runicast_conn *c = (struct runicast_conn *)stunicast;
	mov	r0,sp
	dec	r0
	dec	r0
	mov	@r0,ar5
	...
	inc	r0
	mov	@r0,ar7
;	../../core/net/rime/runicast.c:81: if(packetbuf_attr(PACKETBUF_ATTR_PACKET_TYPE) == PACKETBUF_ATTR_PACKET_TYPE_DATA) {
	mov	dpl,#0x0e
	...
	ljmp	00109$
00127$:
;	../../core/net/rime/runicast.c:83: c->rxmit += 1;
	mov	r0,sp
	dec	r0
	dec	r0
	mov	a,#0x34
	...
	mov	a,r7
	lcall	__gptrput
;	../../core/net/rime/runicast.c:85: if(c->rxmit != 0) {
	mov	dpl,r2
	mov	dph,r3
	mov	b,r4
	lcall	__gptrget
	jz	00102$
;	../../core/net/rime/runicast.c:86: RIMESTATS_ADD(rexmit);
	push	ar2
	push	ar3
	push	ar4
...
00129$:
	...
	inc	dptr
	movx	@dptr,a
;	../../core/net/rime/runicast.c:103: c->sndnxt = (c->sndnxt + 1) % (1 << RUNICAST_PACKET_ID_BITS);
	pop	ar4
	pop	ar3
	pop	ar2
;	../../core/net/rime/runicast.c:86: RIMESTATS_ADD(rexmit);
00102$:
;	../../core/net/rime/runicast.c:92: if(c->rxmit >= c->max_rxmit) {
	mov	dpl,r2
	mov	dph,r3
	mov	b,r4
	lcall	__gptrget
	...
	ljmp	00109$
00130$:
;	../../core/net/rime/runicast.c:93: RIMESTATS_ADD(timedout);
...
00131$:
	...
	inc	dptr
	movx	@dptr,a
;	../../core/net/rime/runicast.c:94: c->is_tx = 0;
	mov	r0,sp
	dec	r0
	dec	r0
	mov	a,#0x33
	...
	clr	a
	lcall	__gptrput
;	../../core/net/rime/runicast.c:95: stunicast_cancel(&c->c);
	mov	r0,sp
	dec	r0
	dec	r0
	mov	dpl,@r0
...
;	../../core/net/rime/runicast.c:96: if(c->u->timedout) {
	mov	r0,sp
	dec	r0
	dec	r0
	mov	a,#0x2f
	...
	orl	a,r6
	jz	00104$
;	../../core/net/rime/runicast.c:97: c->u->timedout(c, stunicast_receiver(&c->c), c->rxmit);
	mov	r0,sp
	dec	r0
	dec	r0
	mov	a,#0x34
...
00134$:
	...
	pop	ar7
00104$:
;	../../core/net/rime/runicast.c:99: c->rxmit = 0;
	mov	r0,sp
	dec	r0
	dec	r0
	mov	a,#0x34
...
;max_retransmissions       Allocated to stack - sp -8
;c                         Allocated to stack - sp -2
